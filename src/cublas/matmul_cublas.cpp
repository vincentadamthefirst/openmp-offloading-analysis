...
void run(std::string path, bool isCsv, bool verbose, uint32_t repetitions, uint32_t mStart, uint32_t mEnd, bool tensor,
    ...
    for (uint32_t matrixSize = mStart; matrixSize <= mEnd; matrixSize *= 2) {
        ...
        LOG("Matrices of size ", matrixSize, std::endl);
        ...
        for (uint32_t repetition = 0; repetition < repetitions; ++repetition) {
            ...
            if (verbose)
                LOG("  Repetition #", repetition, ": ", runtime, "ms (", Helper::Math::msToGFLOPs(runtime, matrixSize),
                    " GFLOP/s)", std::endl);
        }
        ...
        if (verbose)
            LOG("  RESULTS: ", info, std::endl, std::endl);
